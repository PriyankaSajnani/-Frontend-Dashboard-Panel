var bt = {
  ...
  check_domain: function (
    ...
    var reg =
      /^([\w\u4e00-\u9fa5\-\*]{1,100}\.){1,4}([\w\u4e00-\u9fa5\-]{1,24}|[\w\u4e00-\u9fa5\-]{1,24}\.[\w\u4e00-\u9fa5\-]{1,24})$/;
  ...
  select_path: function (id, type) {
    ...
    function ActiveDisk() {
      ...
      switch (a) {
        case "C":
          $(".path-con-left dd:nth-of-type(1)")
            .css("background", "#eee")
            .siblings()
            .removeAttr("style");
          break;
        case "D":
          $(".path-con-left dd:nth-of-type(2)")
            .css("background", "#eee")
            .siblings()
            .removeAttr("style");
          break;
        case "E":
          $(".path-con-left dd:nth-of-type(3)")
            .css("background", "#eee")
            .siblings()
            .removeAttr("style");
          break;
        case "F":
          $(".path-con-left dd:nth-of-type(4)")
            .css("background", "#eee")
            .siblings()
            .removeAttr("style");
          break;
        case "G":
          $(".path-con-left dd:nth-of-type(5)")
            .css("background", "#eee")
            .siblings()
            .removeAttr("style");
          break;
        case "H":
          $(".path-con-left dd:nth-of-type(6)")
            .css("background", "#eee")
            .siblings()
            .removeAttr("style");
          break;
    ...
    bt.send(
      ...
      function (rdata) {
        ...
        for (var f = 0; f < rdata.DIR.length; f++) {
          ...
          d +=
            '<tr><td><input type="checkbox" /></td><td class="bt_open_dir" path ="' +
            rdata.PATH +
            "/" +
            g[0] +
            '" data-type="dir" title=\'' +
        ...
        $(".bt_open_dir span").click(function () {
          if ($(this).parent().data("type") == "dir")
            _that.get_file_list($(this).parent().attr("path"), type);
        });
      }
    );
  },
  ...
  to_login: function () {
    layer.confirm(
      lan.public_backup.login_expire,
      {
        title: lan.public_backup.session_expire,
        icon: 2,
        closeBtn: 1,
        shift: 5,
      },
  ...
  render_form_line: function (item, bs, form) {
    ...
    if (item.items && item.type != "select") {
      for (var x = 0; x < item.items.length; x++) {
        ...
        switch (_obj.type) {
          ...
          case "textarea":
            ...
            if (_placeholder)
              _html +=
                '<div class="placeholder c9" style="top: 12px; left: 14px; display: block;">' +
                _placeholder +
                "</div>";
            break;
          case "button":
        ...
        if (_obj.callback)
          clicks.push({ bind: _name + bs, callback: _obj.callback });
        if (_obj.event) {
          _html +=
            '<span data-id="' +
            _name +
      ...
      if (item.callback)
        clicks.push({ bind: _name + bs, callback: item.callback });
    ...
    _html =
      '<div class="line ' + item.class + '" ' + _hide + ">" + _html + "</div>";
        ...
        if (item) {
          ...
          if (item.help)
            thead +=
              '<a href="' +
              item.help +
              '" class="bt-ico-ask" target="_blank" title="' +
              lan.public_backup.click_detail +
              '">?</a>';
  ...
  saveEditor: function (ace) {
    if (!ace.saveCallback) {
      ...
      bt.send(
        "SaveFileBody",
        "files/SaveFileBody",
        { data: ace.ACE.getValue(), path: ace.path, encoding: "utf-8" },
        function (rdata) {
          loadT.close();
          bt.msg(rdata);
        }
...
bt.pub = {
    ...
    bt.confirm(
      {
        ...
        title: typeName + serverName + lan.public_backup.server,
      },
      function () {
        ...
        bt.send("system", "system/ServiceAdmin", data, function (rdata) {
          load.close();
          var f = rdata.status
            ? lan.get("service_ok", [serverName, msg])
            : lan.get("service_err", [serverName, msg]);
  ...
  bind_btname: function (callback) {
    layer.open({
      ...
      content:
        ...
        '" type="button" ></div><p class="text-right"><a class="btlink" href="https://brandnew.aapanel.com/user_admin/login" target="_blank">' +
  ...
  unbind_bt: function () {
    ...
    bt.confirm(
      { msg: lan.config.binding_un_msg, title: lan.config.binding_un_title },
      function () {
  ...
  on_edit_file: function (type, fileName) {
    if (type != 0) {
      ...
      bt.send(
        "SaveFileBody",
        "files/SaveFileBody",
        "data=" + body + "&path=" + fileName + "&encoding=" + encoding,
        function (rdata) {
    ...
    bt.send("GetSoftList", "ajax/GetSoftList", {}, function (l) {
      ...
      for (var h = 0; h < l.length; h++) {
        ...
        var m =
          ...
          "' type='checkbox' " +
          (l[h].name == "DNS-Server" || l[h].name == "Mail-Server"
            ? ""
            : "checked") +
          ">";
      ...
      g = g
        .replace(new RegExp(/(data_)/g), "apache_")
        .replace(new RegExp(/(select_)/g), "apache_select_");
      var k = layer.open({
        type: 1,
...
bt.weixin = {
  ...
  init: function () {
    ...
    $(".bt-w-menu").height(
      _this.relWidth - 1 - $(".layui-layer-title").height()
    );
...
bt.ftp = {
  get_list: function (page, search, callback) {
    ...
    var order = bt.get_cookie("order")
      ? "&order=" + bt.get_cookie("order")
      : "";

    var data =
      "tojs=ftp.get_list&table=ftps&limit=15&p=" +
      page +
      "&search=" +
      search +
      order;
        ...
        var _tab = bt.render({
          ...
          columns: [
            ...
            },
            {
              ...
              templet: function (item) {
                var opt =
                  '<a class="btlink" href="javascript:;" onclick="bt.recycle_bin.re_recycle_bin(\'' +
                  item.rname +
                  "'," +
                  type +
                  ')">' +
                  lan.public_backup.recover +
                  "</a> | ";
                opt +=
                  '<a class="btlink" href="javascript:;" onclick="bt.recycle_bin.del_recycle_bin(\'' +
                  item.rname +
                  "'," +
                  type +
                  ')">' +
                  lan.public_backup.permanent_delete +
                  "</a>";
                return opt;
              },
            },
...
bt.files = {
  ...
  get_files: function (Path, searchV, callback) {
    ...
    bt.send(
      "get_files",
      "files/GetDir",
      "tojs=GetFiles&p=" + p + "&showRow=" + showRow + search + "&path=" + Path,
      function (rdata) {
        loadT.close();
  ...
  del_recycle_bin: function (path, callback) {
    bt.confirm(
      {
        msg: lan.files.recycle_bin_del_msg,
  ...
  clear_recycle_bin: function (callback) {
    bt.confirm(
      {
        msg: lan.files.recycle_bin_close_msg,
        title: lan.files.recycle_bin_close,
      },
      function () {
        var loadT = bt.load(
          "<div class='myspeed'>" + lan.files.recycle_bin_close_the + "</div>"
        );
        bt.send(
          "Re_Recycle_bin",
  ...
  rename: function (fileName, type, callback) {
    ...
    if (type) {
      ...
      bt.send(
        "MvFile",
        "files/MvFile",
        "sfile=" + oldFileName + "&dfile=" + newFileName,
        function (rdata) {
          loading.close();
          bt.msg(rdata);
          if (callback) callback(rdata);
        ...
        if (rdata.length > 0) {
          ...
          for (var i = 0; i < rdata.length; i++) {
            tbody +=
              ...
              rdata[i].filename +
              "</td><td>" +
              bt.format_size(rdata[i].size) +
              "</td><td>" +
              bt.format_data(rdata[i].mtime) +
              "</td></tr>";
          }
          var mbody =
  ...
  paste_to: function (path, copyName, cutName, fileName, callback) {
    if (copyName != "null" && copyName != undefined) {
      ...
      bt.send(
        "CopyFile",
        "files/CopyFile",
        "sfile=" + copyName + "&dfile=" + path + "/" + fileName,
        function (rdata) {
          loading.close();
          bt.msg(rdata);
          if (callback) callback(rdata);
  ...
  zip: function (dirName, submits, callback) {
    ...
    if (submits != undefined) {
      ...
      bt.send(
        "Zip",
        "files/Zip",
        "sfile=" + sfile + "&dfile=" + dfile + "&type=tar&path=" + path,
        function (rdata) {
          loading.close();
  ...
  show_img: function (fileName) {
    ...
    bt.open({
      ...
      shadeClose: true,
      content:
        '<div class="showpicdiv"><img width="100%" src="' + imgUrl + '"></div>',
    });
  ...
  set_chmod: function (action, fileName, callback) {
    ...
    var toExec =
      fileName == lan.files.all
        ? "Batch(3,1)"
        : "_this.set_chmod(1,'" + fileName + "',callback)";
  ...
  download_file: function (action, callback) {
    ...
    if (action == 1) {
      ...
      bt.send(
        "DownloadFile",
        "files/DownloadFile",
        "path=" + fpath + "&url=" + fUrl + "&filename=" + fname,
        function (rdata) {
          loading.close();
          bt.msg(rdata);
          if (callback) callback(rdata);
...
bt.crontab = {
  ...
  del_task_send: function (id, name) {
    bt.show_confirm(
      lan.public_backup.del + "[" + name + "]",
      lan.public_backup.del_task,
      function () {
        bt.send(
          "del_task_send",
          "crontab/DelCrontab",
  ...
  set_crontab_status: function (id, status, callback) {
    ...
    bt.confirm(
      {
        title: lan.public_backup.tips,
        msg: status
          ? lan.public_backup.stop_crontab
          : lan.public_backup.start_crontab,
      },
      function () {
        bt.send(
          "set_crontab_status",
    ...
    bt.send(
      ...
      function (rdata) {
        that.backupsList.siteList = [
          { name: "ALL", ps: lan.public_backup.all },
        ];
        that.backupsList.optList = [
          { name: lan.public_backup.server_disk, value: "localhost" },
        ];
        that.backupsList.siteList = that.backupsList.siteList.concat(
          rdata.data
        );
        that.backupsList.optList = that.backupsList.optList.concat(
          rdata.orderOpt
        );
        that.initFrom["crontab-name"] =
          name +
          "[" +
          that.backupsList.siteList[that.initFrom["crontab-site"]].name +
          "]";
...
bt.control = {
  ...
  clear_control: function (callback) {
    bt.confirm(
      { msg: lan.control.close_log_msg, title: lan.control.close_log },
      function () {
        loadT = bt.load(lan.public.the);
        bt.send(
          "SetControl",
...
bt.firewall = {
  get_log_list: function (page, search, callback) {
    ...
    var data =
      "tojs=firewall.get_log_list&table=logs&limit=10&p=" +
      page +
      "&search=" +
      search +
      order;
  ...
  get_list: function (page, search, callback) {
    ...
    var data =
      "tojs=firewall.get_list&table=firewall&limit=10&p=" +
      page +
      "&search=" +
      search +
      order;
  ...
  set_mstsc: function (port, callback) {
    bt.confirm(
      { msg: lan.firewall.ssh_port_msg, title: lan.firewall.ssh_port_title },
      function () {
        loading = bt.load(lan.public.the);
        bt.send(
          "SetSshPort",
  ...
  del_accept_port: function (id, port, callback) {
    ...
    bt.confirm(
      { msg: lan.get("confirm_del", [port]), title: lan.firewall.del_title },
      function (index) {
  ...
  clear_logs: function (callback) {
    bt.confirm(
      { msg: lan.firewall.close_log_msg, title: lan.firewall.close_log },
      function () {
        var loadT = bt.load(lan.firewall.close_the);
...
bt.soft = {
  ...
  php: {
      ...
      bt.confirm(
        {
          ...
          title: lan.public_backup.install + "【" + name + "】",
        },
        function () {
          ...
          bt.send(
            "InstallSoft",
            "files/InstallSoft",
            { name: name, version: version, type: "1" },
            function (rdata) {
              loadT.close();
              if (callback) callback(rdata);
              bt.msg(rdata);
      ...
      bt.confirm(
        {
          ...
          title: lan.public_backup.uninstall + "【" + name + "】",
        },
        function () {
          name = name.toLowerCase();
    ...
    disable_functions: function (version, fs, callback) {
      ...
      bt.send(
        ...
        { version: version, disable_functions: fs },
        function (rdata) {
          loadT.close();
          if (callback) callback(rdata);
        }
  ...
  pro: {
    ...
    get_product_discount_by: function (product_id, callback) {
      if (product_id) {
        ...
        bt.send(
          "get_product_discount_by",
          "auth/get_product_discount_by",
          {},
          function (rdata) {
            if (callback) callback(rdata);
          }
    ...
    create_order: function (data, callback) {
      if (data.pid) {
        ...
        bt.soft.get_panel_ssl_status(function (res) {
          loadT.close();
          if (res.status) {
            var _cycle_unit = $("#libPay-content .li-con .active  span").attr(
                "data-unit"
              ),
              _pay_channel =
                $("#libPay-mode .pay-cycle-btn span").text().indexOf("Stripe") >
                -1
                  ? "2"
                  : "10";
            requestNmae = data.serial_no
              ? "renew_product_auth"
              : "get_buy_code";
            ...
            $("#libPay-pay").empty().append(
              '<div style="font-size: 18px;position: relative;top: 25px;text-align: center;">Please apply for SSL before purchasing!</div>\
      ...
      success: function (indexs, layers) {
        ...
        if (!bt_user_info) {
          bt.pub.get_user_info(function (res) {
            $(".bindUser span").html(
              res.data.username +
                '<a href="javascript:;" class="btlink ml5">Change</a>'
            );
          });
        } else {
          $(".bindUser span").html(
            JSON.parse(bt_user_info).data.username +
              '<a href="javascript:;" class="btlink ml5">Change</a>'
          );
        }
        endTime != null
          ? $(".endTime span").html(
              endTime > parseInt(new Date().getTime() / 1000)
                ? bt.format_data(endTime)
                : '<i style="color:red;font-style:inherit">Expired</i>'
            )
          : $(".endTime").hide();
        ...
        if (config.plugin)
          arry.push({
            ...
            active:
              (config.pro < 0 && config.ltd < 0) ||
              (config.type == 12 && config.ltd < 0)
                ? true
                : false,
          });
        if (
          (((config.pro > 0 || config.pro == -2 || config.ltd < 0) &&
            ((config.ltd > 0 && config.ltd != config.pro) || config.ltd < 0) &&
            config.type != 12) ||
            config.limit == "pro" ||
            (config.ltd < 0 && config.pro == -1)) &&
          config.type != 12 &&
          ((config.ltd < 0 && config.pro > 0) ||
            (config.ltd < 0 && config.pro < 0))
        ) {
          arry.push({
    ...
    switch (condition) {
          ...
          function (res) {
            bt.soft.pro.get_voucher(config.pid, function (rdata) {
              ...
              if (config.renew) {
                ...
                _arry[
                  res.length > 0 ? "3" : rdata.length > 0 ? "2" : "1"
                ].active = true;
                config.condition =
                  res.length > 0 ? 7 : rdata.length > 0 ? 4 : 2;
              }
              if (res == null) res = [];
              if (rdata == null) rdata = [];
              $("#libPay-mode .li-con")
      ...
      case 5:
        ...
        if ($("#libPay-pay").attr("data-qecode")) {
          ...
          $("#libPay-pay")
            ...
            .css(
              "display",
              config.sprice > config.price ? "inline-block" : "none"
            )
            .html("$ " + config.sprice.toFixed(2));
          $("#libPay-pay")
            .find("#PayQcode")
            .html('<div class="loading">Loading, please wait!</div>');
          if (qcode) {
        ...
        if (bt.get_cookie("serial_no") != null)
          paream.serial_no = config.serial_no || bt.get_cookie("serial_no");
        that.pro.create_order(paream, function (rdata) {
          ...
          config.pay = parseInt(
            $("#libPay-mode .pay-cycle-btn.active").data("condition")
          );
          // 二维码显示界面
          $("#libPay-pay")
            .empty()
            .append(
              that.product_pay_swicth(
                config.pay == 2 ? "wechat" : "alipay",
                $.extend(
                  {
        ...
        $("#libPay-pay").on("click", "#checkout-button", function () {
          ...
          that.pro.get_check_out_info($(this).data("code"), function (res) {
            ...
            if (res.id) {
              ...
              layer.msg("Payment order failed, please contact administrator!", {
                icon: 2,
              });
            }
          });
      ...
      case 6:
        var _html = $('<div class="paymethod-submit text-center"></div>'),
          _button = $(
            '<button class="btn btn-success btn-sm f16 ' +
              (config.serial_no ? "" : "disabled") +
              '" style="width: 200px; height: 40px;">' +
              (config.serial_no ? "Pay" : "No vouchers") +
              "</button>"
          );
        _button.click(function (ev) {
          ...
          bt.soft.pro.create_order_voucher(
            config.pid,
            config.code,
            config.id,
            config.cycle,
        ...
        bt.send(
          ...
          function (res) {
            ...
            if (res.length == 0) {
              _html.append(
                $(
                  '<li class="pay-cycle-btn" id="no_authorization" style="width:180px;cursor: default;" disabled="disabled"><span> No authorization</span></li>'
                )
              );
            }
            ...
            $("#libPay-pay")
              ...
              .append(
                $(
                  '<div class="lib-price-box text-center"><button type="button" id="authorization" style="margin-top:30px" class="btn btn-success ">Authorization</button></div>'
                )
              );
            //授权
            $("#authorization").unbind();
            if (
              $("#libPay-content .li-con ul li  span")
                .html()
                .indexOf("No authorization") > 0
            ) {
        ...
        this.each(config.data, function (index, item) {
          _html.append(
            $(
              ...
              .data($.extend({ pid: config.pid, dom_index: index }, item))
              .click(function (ev) {
                var data = $(this).data();
                if (!$(this).hasClass("active"))
                  that.product_pay_page_refresh(
                    $.extend({ condition: 5 }, data)
                  );
                $(this).addClass("active").siblings().removeClass("active");
              })
          );
          ...
          _html.append(
            $(
              '<li class="pay-cycle-btn ' +
                (item.active ? "active" : "") +
                '"><span>' +
                (item.cycle_unit == "month" && item.cycle == 999
                  ? "永久"
                  : item.cycle + that.pro.conver_unit(item.cycle_unit)) +
                "</span></li>"
            )
  ...
  get_rscode_other: function (pid, price, cycle, type) {
    var loadT = layer.msg(lan.public_backup.get_payment_info, {
      icon: 16,
      time: 0,
      shade: 0.3,
    });
  ...
  get_voucher_list: function (pid) {
    $("#couponlist").html(
      "<div class='cloading'>" + lan.public_backup.loading + "</div>"
    );
      ...
      if (rdata != null && rdata.length > 0) {
        ...
        for (var i = 0; i < len; i++) {
          if (rdata[i].status != 1) {
            ...
            con +=
              '<li class="pay-cycle-btn" data-coupon-id ="' +
              rdata[i].id +
              '" data-charge-type ="' +
              rdata[i].charge_type +
              '"  data-code="' +
  ...
  get_rscode: function (pid, price, sprice, cycle) {
    ...
    $(".pay-wx").html(
      '<span class="loading">' + lan.public_backup.loading + "</span>"
    );
    $(".libPay").append(
      '<div class="payloadingmask" style="height:100%;width:100%;position:absolute;top:0;left:0;z-index:1"></div>'
    );
        ...
        bt.soft.pub.wxpayTimeId = setInterval(function () {
          bt.soft.pro.get_plugin_coupon(pid, function (rdata) {
            if (rdata.status) {
              ...
              clearInterval(bt.soft.pub.wxpayTimeId);
              bt.msg({
                msg: lan.public_backup.pay_plug_success,
                icon: 16,
                time: 0,
                shade: [0.3, "#000"],
              });
        ...
        bt.soft.pub.wxpayTimeId = setInterval(function () {
          bt.soft.pro.get_re_order_status(function (rdata) {
            if (rdata.status) {
              ...
              clearInterval(bt.soft.pub.wxpayTimeId);
              bt.msg({
                msg: lan.public_backup.pay_pro_success,
                icon: 16,
                time: 0,
                shade: [0.3, "#000"],
              });
      ...
      if (rdata != null) {
        ...
        for (var i = 0; i < qlen; i++) {
          ...
          coucon +=
            ...
            (c == 1
              ? ""
              : "<em>" + c * 10 + lan.public_backup.discount + "</em>") +
            "</li>";
        }
        $(".libpay-con").html(con);
  ...
  get_soft_list: function (p, type, search, callback) {
    ...
    bt.send(
      ...
      { p: p, type: type, tojs: "soft.get_list", force: force, query: search },
      function (rdata) {
        if (loading) loading.close();
  ...
  show_make_args: function (name) {
    ...
    bt.soft.loadOpen = bt.open({
      ...
      success: function (layer, index) {
        ...
        _aceEditor.setValue(
          "# The shell script content executed before compilation is usually prepared for the dependent installation and source download of the third-party module"
        );
      },
      yes: function () {
  ...
  set_make_args: function (_this, name, args_name) {
    ...
    bt.send(
      ...
      { name: name, args_names: bt.soft.make_config.join("\n") },
      function (rdata) {
        if (!rdata.status) {
          bt.msg(rdata);
        }
  ...
  del_make_args: function (name, args_name) {
    ...
    bt.confirm(
      {
        msg: "Confirm delete[" + name + ":" + args_name + "]module？",
        title: "Delete[" + name + ":" + args_name + "]module!",
      },
      function () {
    ...
    bt.send(
      ...
      function (rdata) {
        ...
        for (var i = 0; i < rdata.args.length; i++) {
          ...
          var checked_str =
            bt.soft.make_config.indexOf(rdata.args[i].name) == -1
              ? ""
              : 'checked="checked"';
          module_html +=
            '<tr>\
  ...
  install: function (name, that) {
    ...
    if (bt.soft.is_install) {
      layer.msg("Installing other software, please operate later!", {
        icon: 0,
      });
      return false;
    }
      ...
      if ($.inArray(name, arrs) >= 0 || name.indexOf("php-") >= 0) {
        ...
        if (rdata.versions.length > 1) {
          for (var i = 0; i < rdata.versions.length; i++) {
            ...
            SelectVersion +=
              "<option>" + name + " " + item.m_version + "</option>";
          }
          shtml =
  ...
  loadT: null,
  speed_msg:
    "<pre style='margin-bottom: 0px;height:250px;text-align: left;background-color: #000;color: #fff;white-space: pre-wrap;' id='install_show'>[MSG]</pre>",
  //显示进度窗口
  ...
  install_soft: function (item, version, type, that) {
    ...
    layer.confirm(
      item.type != 5
        ? lan.soft.lib_insatll_confirm.replace("{1}", item.title)
        : lan.get("install_confirm", [item.title, version]),
      {
      ...
      function () {
        ...
        bt.soft.show_speed_window(
          ...
          function () {
            if (item.type == 10)
              loadT = layer.msg(
                'Getting third party installation information, please wait<img src="/static/img/ing.gif">',
                { icon: 16, time: 0, shade: [0.3, "#000"] }
              );
            bt.send(
              "install_plugin",
              "plugin/install_plugin",
              { sName: item.name, version: version, type: type },
              function (rdata) {
                if (rdata.size) {
                  layer.close(loadT);
  ...
  install_other: function (data) {
    ...
    var loadT = layer.open({
      ...
      area: "500px",
      title:
        (data.update ? lan.public_backup.update : lan.public_backup.install) +
        lan.public_backup.third_party_plug,
      closeBtn: 2,
      shift: 5,
      shadeClose: false,
      btn: [
        data.update ? lan.public_backup.update : lan.public_backup.install,
        lan.public_backup.cancel,
      ],
      content:
        '<style>\
        ...
        '</a></p>\
                        ...
                        <ul class="help-info-text c7">\
                            ' +
        (data.update
          ? "<li>" + lan.public_backup.update_wait + "</li>"
          : "<li>" +
  ...
  update_soft: function (name, title, version, min_version, update_msg, type) {
    ...
    bt.show_confirm(
      ...
      lan.public_backup.update_tips2
        .replace("{1}", title)
        .replace("{2}", version)
        .replace("{3}", min_version),
      function () {
        ...
        bt.soft.show_speed_window(
          {
            title:
              ...
              "],Please wait...",
            status: true,
            soft: { type: parseInt(type) },
          },
          function () {
            bt.send(
              "install_plugin",
              "plugin/install_plugin",
              { sName: name, version: version, upgrade: version },
              function (rdata) {
                if (rdata.size) {
                  _this.install_other(rdata);
                  return;
  ...
  un_install: function (name) {
    ...
    _this.get_soft_find(name, function (item) {
      ...
      for (var i = 0; i < item.versions.length; i++) {
        if (
          item.versions[i].setup &&
          bt.contains(item.version, item.versions[i].m_version)
        ) {
          version = item.versions[i].m_version;
          if (version.indexOf(".") < 0)
            version += "." + item.versions[i].version;
          break;
        }
      }
      ...
      bt.confirm(
        {
          msg: lan.soft.uninstall_confirm
            .replace("{1}", title)
            .replace("{2}", version),
  ...
  save_config: function (fileName, data) {
    ...
    bt.send(
      "SaveFileBody",
      "files/SaveFileBody",
      { data: data, path: fileName, encoding: encoding },
      function (rdata) {
        loadT.close();
        bt.msg(rdata);
      }
...
bt.database = {
  get_list: function (page, search, callback) {
    ...
    var data =
      "tojs=database.get_list&table=databases&limit=15&p=" +
      page +
      "&search=" +
      search +
      order;
  ...
  get_root_pass: function (callback) {
    bt.send(
      "getKey",
      "data/getKey",
      { table: "config", key: "mysql_root", id: 1 },
      function (rdata) {
        if (callback) callback(rdata);
      }
    );
  ...
  set_data_access: function (name) {
    ...
    bt.send(
      ...
      function (rdata) {
        ...
        setTimeout(function () {
          if (
            rdata.msg.permission == "127.0.0.1" ||
            rdata.msg.permission == "%"
          ) {
  ...
  input_sql: function (fileName, dataName) {
    bt.confirm(
      { msg: lan.database.input_confirm, title: lan.database.input_title },
      function (index) {
        var loading = bt.load(lan.database.input_the);
        bt.send(
          "InputSql",
          "database/InputSql",
          { file: fileName, name: dataName },
          function (rdata) {
            loading.close();
            bt.msg(rdata);
          }
...
bt.site = {
  get_list: function (page, search, type, callback) {
    ...
    var order = bt.get_cookie("order")
      ...
      : "";
    var data =
      "tojs=site.get_list&table=sites&limit=15&p=" +
      page +
      "&search=" +
      search +
      order +
  ...
  set_site_type: function (data, callback) {
    bt.send(
      "set_site_type",
      "site/set_site_type",
      { id: data.id, site_ids: data.site_array },
      function (rdata) {
        if (callback) callback(rdata);
      }
    );
  ...
  add_domains: function (id, webname, domains, callback) {
    ...
    bt.send(
      "AddDomain",
      "site/AddDomain",
      { domain: domains, webname: webname, id: id },
      function (rdata) {
        loading.close();
        if (callback) callback(rdata);
        bt.msg(rdata);
  ...
  add_dirbind: function (id, domain, dirName, callback) {
    ...
    bt.send(
      "AddDirBinding",
      "site/AddDirBinding",
      { id: id, domain: domain, dirName: dirName },
      function (rdata) {
        loading.close();
        if (callback) callback(rdata);
      }
  ...
  set_limitnet: function (id, perserver, perip, limit_rate, callback) {
    ...
    bt.send(
      "SetLimitNet",
      "site/SetLimitNet",
      { id: id, perserver: perserver, perip: perip, limit_rate: limit_rate },
      function (rdata) {
        loading.close();
        if (callback) callback(rdata);
      }
  ...
  remove_redirect: function (sitename, redirectname, callback) {
    bt.show_confirm(
      lan.public_backup.del_rep + "[" + redirectname + "]",
      lan.public_backup.sure_del_rep,
      function () {
        var loadT = bt.load(lan.site.the_msg);
        bt.send(
          "DeleteRedirect",
          "site/DeleteRedirect",
          { sitename: sitename, redirectname: redirectname },
          function (rdata) {
            loadT.close();
            if (callback) callback(rdata);
          }
  ...
  remove_proxy: function (sitename, proxyname, callback) {
    bt.show_confirm(
      lan.public_backup.del_proxy + "[" + proxyname + "]",
      lan.public_backup.sure_del_proxy,
      function () {
        var loadT = bt.load(lan.site.the_msg);
        bt.send(
          "RemoveProxy",
          "site/RemoveProxy",
          { sitename: sitename, proxyname: proxyname },
          function (rdata) {
            loadT.close();
            if (callback) callback(rdata);
            bt.msg(rdata);
  ...
  set_site_301: function (siteName, srcDomain, toUrl, type, callback) {
    ...
    bt.send(
      "Set301Status",
      "site/Set301Status",
      { siteName: siteName, toDomain: toUrl, srcDomain: srcDomain, type: type },
      function (rdata) {
        loading.close();
        if (callback) callback(rdata);
      }
  ...
  create_let: function (data, callback) {
    var loadT = layer.open({
      ...
      offset: "30%",
      content:
        "<pre style='margin-bottom: 0px;height:250px;text-align: left;background-color: #000;color: #fff;white-space: pre-wrap;' id='create_lst'>" +
  ...
  verify_domain: function (partnerOrderId, siteName, callback) {
    ...
    bt.send(
      "Completed",
      "ssl/Completed",
      { partnerOrderId: partnerOrderId, siteName: siteName },
      function (rdata) {
        loadT.close();
        if (callback) callback(rdata);
      }
  ...
  set_cert_ssl: function (certName, siteName, callback) {
    ...
    bt.send(
      "SetCertToSite",
      "ssl/SetCertToSite",
      { certName: certName, siteName: siteName },
      function (rdata) {
        loadT.close();
        site.reload();
        if (callback) callback(rdata);
  ...
  remove_cert_ssl: function (certName, callback) {
    bt.show_confirm(
      ...
      lan.public_backup.sure_del_cert,
      function () {
        var loadT = bt.load(lan.site.the_msg);
        bt.send(
          "RemoveCert",
  ...
  set_ssl: function (siteName, data, callback) {
    if (data.path) {
      ...
      bt.send(
        "SetSSL",
        "site/SetSSL",
        { type: 1, siteName: siteName, key: data.key, csr: data.csr },
        function (rdata) {
          loadT.close();
          if (callback) callback(rdata);
        }
  ...
  add_site: function (callback) {
    ...
    bt.site.get_all_phpversion(function (rdata) {
      bt.site.get_type(function (tdata) {
        ...
        $(".domain_textarea")
          .parents(".bt-form")
          .css({ "max-height": "565px", overflow: "auto" });
      });
    });
  },
  ...
  stop: function (id, name, callback) {
    bt.confirm(
      {
        title: lan.public_backup.stop_site + " 【" + name + "】",
  ...
  start: function (id, name, callback) {
    bt.confirm(
      {
        title: lan.public_backup.start_site + " 【" + name + "】",
  ...
  del_backup: function (id, siteId, siteName) {
    bt.confirm(
      { msg: lan.site.webback_del_confirm, title: lan.site.del_bak_file },
      function (index) {
        var loadT = bt.load();
  ...
  delete_dir_guard: function (id, data, callback) {
    var loading = bt.load();
    bt.show_confirm(
      lan.public_backup.del + "[" + data + "]",
      lan.public_backup.del_dir,
      function () {
        bt.send(
          "delete_dir_auth",
            ...
            if (obj.val() == "ip") {
              obj
                ...
                .append(
                  '<input id="' +
                    subid +
                    '" class="bt-input-text mr5" type="text" name="address" placeholder="' +
                    lan.public_backup.access_ip_tips +
    ...
    data_add: {
      ...
      list: [
        ...
        bt.form.item.data_access,
        {
          ...
          type: "select",
          disabled:
            bt.contains(bt.get_cookie("serverType"), "nginx") ||
            bt.contains(bt.get_cookie("serverType"), "apache")
              ? true
              : false,
          items: [
  ...
  site: {
    add: {
      ...
      list: [
          ...
          items: [
            {
              ...
              callback: function (obj) {
                ...
                if (path.substr(0, dPath.length) == dPath)
                  _path_obj.val(dPath + "/" + ress);
                ...
                bt.setTimeouts = setTimeout(function () {
                  if (bt.check_domain(ress)) {
                    if (ress.indexOf("www.") !== 0) {
                      $(".redirect_checkbox label").html(
                        "Add [<span>www." +
                          ress +
                          "</span>] domain name to the main domain name"
                      );
                    } else if (ress.indexOf("www.") === 0) {
                      $(".redirect_checkbox label").html(
                        "Add <span>" +
                          ress.replace(/^www\./, "") +
                          "</span> to the main domain"
                      );
                    }
                      ...
                      items: [
                        {
                          ...
                          list: [
                            { value: 0, text: "No" },
                            {
                              value: 1,
                              text:
                                'Redirect the main domain name [<span title="' +
                                domain_one +
                                '"> ' +
                                domain_one +
                                '</span>] to [<span title="' +
                            ...
                            },
                            {
                              value: 2,
                              text:
                                'Redirect the [<span title="' +
                                domain +
                                '">' +
                                domain +
                                '</span>] domain name to the main domain [<span title="' +
                                domain_one +
                                '">' +
                                domain_one +
                                "</span>]",
        ...
        {
          title: lan.public_backup.ps,
          name: "ps",
          placeholder: lan.public_backup.site_ps,
        },
        {
              ...
              callback: function (obj) {
                ...
                if (obj.val() != "false") {
                  if (bt.os == "Linux" && obj.val() == "SQLServer") {
                    obj.val("false");
                    bt.msg({
                      msg: lan.public_backup.unsupport_sqlserver,
                      icon: 2,
                    });
                    return;
                  }
                  var _bs = obj.parents("div.bt-form").attr("data-id");
                  var data_user = $('textarea[name="webname"]').data(
                    "database"
                  );
                  var item = {
        ...
        },
        {
          ...
          type: "select",
          name: "type",
          disabled: bt.contains(bt.get_cookie("serverType"), "IIS")
            ? false
            : true,
          items: [
            { value: "PHP", title: "PHP" },
      ...
      btns: [
        { title: lan.public_backup.turn_off, name: "close" },
        {
          ...
          callback: function (rdata, load, callback) {
            ...
            if (!rdata.webname) {
              bt.msg({
                msg: lan.public_backup.domain_format_not_right,
                icon: 2,
              });
              return;
            }
    ...
    set_password: {
      ...
      btns: [
        { title: lan.public_backup.turn_off, name: "close" },
        {
          ...
          callback: function (rdata, load, callback) {
            bt.confirm(
              { msg: lan.ftp.pass_confirm, title: lan.ftp.stop_title },
              function () {
                var loading = bt.load();
                bt.send(
                  "SetUserPassword",
...
var form_group = {
  ...
  select: function (elem) {
    ...
    select_el.find("option").each(function (index, el) {
      ...
      if (active) {
        ...
        _val !== ""
          ? select_val.removeClass("default")
          : select_val.addClass("default");
      }
    });
    select_el.hide();
  ...
  checkbox: function () {
    $('input[type="checkbox"]').each(function (index, el) {
      $(el).hide();
      $(el).after(
        '<div class="bt_checkbox_group ' +
          ($(this).prop("checked") ? "active" : "default") +
          '"></div>'
      );
    });
  ...
  bt.send("get_cert_source", "config/get_cert_source", {}, function (rdata) {
    ...
    var _data = {
      ...
      list: [
          ...
          callback: function (obj) {
            ...
            if (obj.val() == "2") {
              ...
              obj
                .parents("div.line")
                .append(
                  '<div class="line" id=' + subid + ">" + _tr.html + "</div>"
                );
            }
          },
        },
        {
          html:
            '<div class="details"><input type="checkbox" id="checkSSL" /><label style="font-weight: 400;margin: 3px 5px 0px;" for="checkSSL">' +
            lan.config.ssl_open_ps_4 +
        ...
        },
        {
          ...
          callback: function (rdata, load, callback) {
            ...
            var confirm = layer.confirm(
              "Whether to open the panel SSL certificate",
              {
                title: "Tips",
                btn: ["Confirm", "Cancel"],
                icon: 0,
