var soft = {
  ...
  get_list: function (page, type, search) {
    ...
    bt.soft.get_soft_list(page, type, search, function (rdata) {
      ...
      var tBody = "";
      rdata.type.unshift(
        { icon: "icon", id: 0, ps: lan.soft.all, sort: 1, title: lan.soft.all },
        { icon: "icon", id: -1, ps: "Installed", sort: 1, title: "Installed" }
      );
      ...
      if (soft.is_install && soft.is_setup == false) {
        setTimeout(function () {
          soft.get_list(
            bt.get_cookie("load_page") + "not_load",
            bt.get_cookie("load_type"),
            bt.get_cookie("load_search")
          );
        }, 3000);
        soft.is_install = false;
      }
      ...
      columns: [
          ...
          templet: function (item) {
            ...
            if (item.setup) {
              ...
              if (item.admin) {
                ...
                click_opt +=
                  " onclick=\"soft.set_soft_config('" + item.name + "')\" ";
              }
            }
            var is_php5 = item.name.indexOf("php-5") >= 0,
              webcache =
                bt.get_cookie("serverType") == "openlitespeed" ? true : false,
            ...
            if (webcache) {
              ...
              rdata.apache22 &&
              item.name.indexOf("php-") >= 0 &&
              $.inArray(item.name, phps) == -1
            ) {
              click_opt = ' title="' + lan.soft.ap2_2_not_support + '"';
            }
        ...
        },
        {
          ...
          templet: function (item) {
            ...
            if (item.setup) {
              path =
                '<span class="glyphicon glyphicon-folder-open"  onclick="openPath(\'' +
                item.uninsatll_checks +
                "')\"></span>";
            }
            return path;
          },
        ...
        },
        {
          ...
          templet: function (item) {
            ...
            if (item.endtime < 0 && item.pid > 0) {
              ...
              if (item.type != 10) {
                pay_opt =
                  '<a class="btlink" onclick=\'bt.soft.product_pay_view(' +
                  JSON.stringify({
                    name: item.title,
                    pid: item.pid,
                    type: item.type,
            ...
            if (webcache && is_php) {
              if (
                (is_php5 || item.name == "php-7.0") &&
                distribution == "centos8"
              ) {
                option =
                  "<span title=\"' + lan.soft.ap2_2_not_support + '\">" +
                  lan.soft.not_comp +
                  "</span>";
                ...
                if (distribution != "centos7" && is_php5) {
                  ...
                  if (item.setup && item.task == "1") {
                    if (pay_opt == "") {
                      if (item.versions.length > 1) {
                        for (var i = 0; i < item.versions.length; i++) {
                          ...
                          var ret = bt.check_version(
                            item.version,
                            min_version.m_version + "." + min_version.version
                          );
                          if (ret > 0) {
                            if (ret == 2)
                              option +=
                    ...
                    option =
                      '<a class="btlink" onclick="messagebox()"  >Updating</a>';
                    soft.is_install = true;
                  } else {
                    if (pay_opt) {
                      option = pay_opt;
      ...
      layer.open({
        ...
        content: (function () {
          ...
          for (var i = 0; i < find_data.length; i++) {
            ...
            if (!item.setup) {
              thtml =
                '<button type="button" class="btn btn-success btn-xs" onclick="bt.soft.install(\'' +
                item.name +
                "',this)\">Install</button>";
            } else {
              if (item.pid != 0) {
                if (item.endtime == 0) {
                  //永久
                  thtml =
                    '<button type="button" class="btn btn-success btn-xs" onclick="bt.soft.set_lib_config(\'' +
                    item.name +
                    "','" +
                    item.title +
                    "')\">Setting</button>";
  ...
  set_soft_tips: function (el, type) {
    var tips_info = $(
        '<div class="alert" style="margin-bottom:15px"><div class="soft_tips_text"></div><div class="btn-ground" style="display:inline-block;"></div></div>'
      ),
    ...
    if (type == 10) {
      ...
      explain.text(
        "BT one click宝塔一键部署已上线，诚邀全球优秀项目入驻(限项目官方) "
      );
      btn_ground = soft.render_tips_btn(btn_ground, [
        {
          title: "免费入驻",
      ...
      if (
        ...
        _ltd =
          (ltd > 0 && (ltd == pro || pro < 0)) || (ltd > 0 && pro >= 0) ? 1 : 0;
        explain.html(
          "The " +
            (_ltd ? "Pro" : "Pro") +
            " edition can use the " +
              ...
              ? "expiration time: " +
                ...
                ((_ltd ? ltd : pro) - todayDate <= 15 * 24 * 60 * 60
                  ? '，<span style="color:red">Only ' +
                    Math.round(
                      ((_ltd ? ltd : pro) - todayDate) / (24 * 60 * 60)
                    ) +
                    " days until expiration</span>"
                  : "")
              : '，过期时间：<span style="color: #fc6d26;font-weight: bold;">永久授权</span>')
        );
      } else if (ltd == -1 && pro == -1) {
        ...
        explain.html(
          "The Pro edition can use the professional plug-in for free, expiration time: 永久授权。" +
            (type == 12
              ? '&nbsp;&nbsp;<span style="color:#af8e48">升级企业版，企业可以免费试用企业版插件及专业版插件。</span>'
              : "")
        );
        if (type == 12) {
          btn_ground = soft.render_tips_btn(btn_ground, {
      ...
      var set_btn_style = function (res) {
        if (!res.status || !res) {
          ...
          if (type == 12 && ltd < 0 && pro >= 0) {
            ...
            $.extend(btn_config, {
              title: _ltd == null ? "Upgrade now" : "Renew Now",
              btn: _ltd == null ? "Upgrade now" : "Renew Now",
              click: fun,
            });
          }
        }
        ...
        $(el).append(
          tips_info.addClass(_ltd == 1 ? "alert-ltd-success" : "alert-success")
        );
        if (_this.trail) {
          setTimeout(function () {
            $(".btn-ground").after(
              '<span class="pro_trail" style="font-weight: 700;margin-left:25px;">Try the Pro edition for free for 15 days</span>'
            );
            var trail = $(
              '<a href="javascript:;" class="btn btn-success btn-xs va0 ml15" style="margin-left:10px;">Click to try</a>'
            );
            trail.click(
              !res.status || !res
                ? fun
  ...
  render_tips_btn: function (node, arry) {
    ...
    for (var i = 0; i < arry.length; i++) {
      var item = arry[i],
        btn = "<a ";
      for (var key in item) {
        if (key != "click" && key != "btn")
          btn += item[key] ? key + '="' + item[key] + '" ' : "";
      }
      btn += ">" + item["btn"] + "</a>";
      if (item.click) {
    ...
    $.post("/deployment?action=GetList", pdata, function (rdata) {
      ...
      if ($(".onekey-type").attr("class") === undefined) {
        ...
        for (var i = 0; i < rdata.dep_type.length; i++) {
          ...
          tbody +=
            '<span typeid="' +
            rdata.dep_type[i].tid +
            '" ' +
            c +
            ">" +
            rdata.dep_type[i].title +
            "</span>";
        }
        tbody += "</div>";
        ...
        zbody +=
          ...
          (rdata.list[i].sort !== undefined
            ? '<a href="javascript:;" class="btlink open_score_view" onclick="score.open_score_view(' +
              ...
              rdata.list[i].count +
              ')" >' +
              (rdata.list[i].sort <= 0 || rdata.list[i].sort > 5
                ? "No rating"
                : rdata.list[i].sort.toFixed(1)) +
              "</a>"
            : "--") +
          "</td>" +
  ...
  remove_other_dep: function (name) {
    bt.show_confirm(
      lan.soft.del_custom_item,
      lan.soft.confirm_del.replace("{1}", name),
      function () {
  ...
  input_package_to: function () {
    ...
    if (
      ...
      layer.msg(
        "The following are required (Index name / Name / Project version / PHP version / Introduction)",
        {
          icon: 2,
        }
      );
      ...
      if (arrs) {
        for (var i = 0; i < arrs.length; i++) {
          ...
          if (item) {
            ...
            meun +=
              "<p onclick=\"soft.get_tab_contents('" +
              item.type +
              '\',this)" title="' +
              tit +
              '">' +
  ...
  set_soft_config: function (name) {
    ...
    bt.soft.get_soft_find(name, function (rdata) {
      ...
      setTimeout(function () {
        menu.append(
          $(
            '<p class="bgw bt_server" onclick="soft.get_tab_contents(\'service\',this)">' +
              lan.soft.service +
              "</p>"
          )
        );
          ...
          for (var i = 0; i < phpSort.length; i++) {
            ...
            if (item) {
              if (item.os == undefined || item["os"] == bt.os) {
                ...
                if (
                  webcache &&
                  (item.type == "set_fpm_config" ||
                    item.type == "get_php_status")
                )
                  continue;
  ...
  get_tab_contents: function (
    ...
    switch (key) {
      case "service":
        ...
        if (data.name == "phpmyadmin") {
          ...
          for (var i = 0; i < status_list.length; i++)
            btns.append(
              '<button class="btn btn-default btn-sm" onclick="bt.pub.set_server_status(\'' +
                data.name +
                "','" +
                status_list[i].opt +
                "')\">" +
        ...
        var help =
          '<ul class="help-info-text c7 mtb15" style="padding-top:30px"><li>' +
      ...
      case "config":
        var tabCon = $(".soft-man-con").empty();
        tabCon.append(
          '<p style="color: #666; margin-bottom: 7px">' +
            lan.bt.edit_ps +
            "</p>"
        );
        tabCon.append(
        ...
        tabCon.append(
          '<button id="OnlineEditFileBtn" class="btn btn-success btn-sm" style="margin-top:10px;">' +
            lan.public.save +
            "</button>"
        );
      ...
      case "fpm_config":
        ...
        tabCon.append(
          '<div class="bt-input-text ace_config_editor_scroll" style="line-height:18px;" id="textBody"></div>'
        );
        tabCon.append(
          '<button id="OnlineEditFileBtn" class="btn btn-success btn-sm" style="margin-top:10px;">' +
            lan.public.save +
        ...
        var _arry = [
          "If you do not understand the php-fpm configuration file, please do not modify it!",
        ];
        tabCon.append(bt.render_help(_arry));
        $(".return_php_info").click(function () {
          $(".bt-soft-menu p:eq(12)").click();
        });
        var fileName = bt.soft
          .get_config_path(version)
          .replace("php.ini", "php-fpm.conf");
        var loadT = bt.load(lan.soft.get);
      ...
      case "change_version":
        ...
        for (var i = 0; i < data.versions.length; i++) {
          if (data.versions[i].setup)
            opt_version = data.name + " " + data.versions[i].m_version;
          _list.push({
            ...
            },
            {
              ...
              callback: function (ldata) {
                ...
                if (data.name == "mysql") {
                  ...
                  $.post(
                    "/plugin?action=check_install_limit",
                    pdata,
                    function (rdata) {
                      ...
                      bt.database.get_list(1, "", function (ddata) {
                        ...
                        bt.soft.install_soft(
                          data,
                          ldata.phpVersion.split(" ")[1],
                          0
                        );
                      });
                    }
      ...
      case "change_mysql_ssl":
        bt.send(
          ...
          function (rdata) {
            ...
            $(".soft-man-con").append(
              bt.render_help([
                "After setting, manually restart the database to take effect",
                "Download Mysql SSL self-signed certificate<a class='btlink downssl' href='javascript:;'>【SSL.zip】</a>",
              ])
            );
            ...
            $("#write_ssl").change(function () {
              ...
              $.post("/database?action=write_ssl_to_mysql", function (rdata) {
                loadT.close(loadT);
                var open_type = $("#write_ssl").prop("checked")
                    ? "turned on"
                    : "turned off",
                  loadP = layer.confirm(
                    "The SSL setting is " +
                      open_type +
                      " successfully.<br> Do you need to restart the database immediately to make it effective?",
                    {
      ...
      case "get_mysql_run_status":
        bt.send("GetRunStatus", "database/GetRunStatus", {}, function (rdata) {
          var cache_size =
            (
              (parseInt(rdata.Qcache_hits) /
                (parseInt(rdata.Qcache_hits) +
                  parseInt(rdata.Qcache_inserts))) *
              100
            ).toFixed(2) + "%";
          if (cache_size == "NaN%") cache_size = "OFF";
          var title10 = (
            (1 - rdata.Threads_created / rdata.Connections) *
            100
          ).toFixed(2);
          var title11 = (
            (1 - rdata.Key_reads / rdata.Key_read_requests) *
            100
          ).toFixed(2);
          var title12 = (
            (1 -
              rdata.Innodb_buffer_pool_reads /
                rdata.Innodb_buffer_pool_read_requests) *
            100
          ).toFixed(2);
          var title14 = (
            (rdata.Created_tmp_disk_tables / rdata.Created_tmp_tables) *
            100
          ).toFixed(2);
          var Con =
            ...
            "</th><td>" +
            parseInt(
              (parseInt(rdata.Com_commit) + parseInt(rdata.Com_rollback)) /
                rdata.Uptime
            ) +
            "</td></tr>\
									<tr><th>" +
      ...
      case "get_mysql_status":
        bt.send("GetDbStatus", "database/GetDbStatus", {}, function (rdata) {
          var key_buffer_size = bt.format_size(
            rdata.mem.key_buffer_size,
            false,
            0,
            "MB"
          );
          var query_cache_size = bt.format_size(
            rdata.mem.query_cache_size,
            false,
            0,
            "MB"
          );
          var tmp_table_size = bt.format_size(
            rdata.mem.tmp_table_size,
            false,
            0,
            "MB"
          );
          var innodb_buffer_pool_size = bt.format_size(
            rdata.mem.innodb_buffer_pool_size,
            false,
            0,
            "MB"
          );
          var innodb_additional_mem_pool_size = bt.format_size(
            rdata.mem.innodb_additional_mem_pool_size,
            false,
            0,
            "MB"
          );
          var innodb_log_buffer_size = bt.format_size(
            rdata.mem.innodb_log_buffer_size,
            false,
            0,
            "MB"
          );

          var sort_buffer_size = bt.format_size(
            rdata.mem.sort_buffer_size,
            false,
            0,
            "MB"
          );
          var read_buffer_size = bt.format_size(
            rdata.mem.read_buffer_size,
            false,
            0,
            "MB"
          );
          var read_rnd_buffer_size = bt.format_size(
            rdata.mem.read_rnd_buffer_size,
            false,
            0,
            "MB"
          );
          var join_buffer_size = bt.format_size(
            rdata.mem.join_buffer_size,
            false,
            0,
            "MB"
          );
          var thread_stack = bt.format_size(
            rdata.mem.thread_stack,
            false,
            0,
            "MB"
          );
          var binlog_cache_size = bt.format_size(
            rdata.mem.binlog_cache_size,
            false,
            0,
            "MB"
          );
          ...
          var form_datas = [
            ...
            },
            {
              ...
              ps:
                "MB, <font>" + lan.soft.mysql_set_query_cache_size + "</font>",
            },
            {
            ...
            },
            {
              ...
              ps:
                "MB, <font>" +
                lan.soft.mysql_set_innodb_buffer_pool_size +
                "</font>",
            },
            {
              ...
              ps:
                "MB, <font>" +
                lan.soft.mysql_set_innodb_log_buffer_size +
                "</font>",
            },
            {
              ...
              ps:
                "KB * " +
                lan.soft.mysql_set_conn +
                ", <font>" +
                lan.soft.mysql_set_sort_buffer_size +
                "</font>",
            },
            {
              ...
              ps:
                ...
                lan.soft.mysql_set_conn +
                ", <font>" +
                lan.soft.mysql_set_read_buffer_size +
                "</font>",
            },
            {
            ...
            },
            {
              ...
              ps:
                ...
                lan.soft.mysql_set_conn +
                ", <font>" +
                lan.soft.mysql_set_join_buffer_size +
                "</font>",
            },
            {
              title: "thread_stack",
              ...
              ps:
                ...
                lan.soft.mysql_set_conn +
                ", <font>" +
                lan.soft.mysql_set_thread_stack +
                "</font>",
            },
            {
              ...
              ps:
                ...
                lan.soft.mysql_set_conn +
                ", <font>" +
                lan.soft.mysql_set_binlog_cache_size +
                "</font>",
            },
            {
          ...
          var tabCon = $(".soft-man-con")
            .empty()
            .append("<div class='tab-db-status'></div>");
          for (var i = 0; i < form_datas.length; i++) {
            bt.render_form_line(form_datas[i], "", $(".tab-db-status"));
          }

          $(
            ".tab-db-status input[name*='size'],.tab-db-status input[name='max_connections'],.tab-db-status input[name='thread_stack']"
          ).change(function () {
            var key_buffer_size = parseInt(
              $("input[name='key_buffer_size']").val()
            );
            var query_cache_size = parseInt(
              $("input[name='query_cache_size']").val()
            );
            var tmp_table_size = parseInt(
              $("input[name='tmp_table_size']").val()
            );
            var innodb_buffer_pool_size = parseInt(
              $("input[name='innodb_buffer_pool_size']").val()
            );
            var innodb_log_buffer_size = parseInt(
              $("input[name='innodb_log_buffer_size']").val()
            );

            var sort_buffer_size =
              $("input[name='sort_buffer_size']").val() / 1024;
            var read_buffer_size =
              $("input[name='read_buffer_size']").val() / 1024;
            var read_rnd_buffer_size =
      ...
      case "mysql_slow_log":
        ...
        bt.send("GetSlowLogs", "database/GetSlowLogs", {}, function (logs) {
          ...
          var phpCon =
            '<textarea readonly="" style="margin: 0px;width: 601px;height: 619px;background-color: #333;color:#fff; padding:0 5px" id="error_log">' +
            logs.msg +
            "</textarea>";
      ...
      case "nginx_format_log":
        ...
        bt.send(
          ...
          "config/get_nginx_access_log_format",
          {},
          function (rdata) {
            $(".soft-man-con").html(
            ...
            bt.send(
              ...
              "config/get_nginx_access_log_format_parameter",
              {},
              function (res) {
                loadT.close();
                ...
                for (const j in rdata) {
                  if (rdata.hasOwnProperty(j)) {
                    ...
                    var element =
                        '<span class="nginx-one-format">' +
                        _format.join('</span><span class="nginx-one-format">') +
                        "</span>",
                      _td =
                        "<tr><td>" +
                        j +
                    ...
                    $("#tab-nginx-logs-format")
                      .parent()
                      .on("scroll", function () {
                        var scrollTop = $("#tab-nginx-logs-format")
                          .parent()
                          .scrollTop();
                        $("#tab-nginx-logs-format thead").css({
                ...
                $(".table-add-format, .table-set-format").click(function () {
                  ...
                  layer.open({
                    ...
                    success: function (index, layero) {
                      $(".nginx-add-format [name=log_format_name]").val(
                        format_name
                      );
                      $(".nginx-add-format")
                      ...
                      $(".nginx-add-format").on(
                        ...
                        function (e) {
                          if ($(this).hasClass("active")) {
                            $(".nginx-add-format .bt-select-list").removeClass(
                              "active"
                            );
                            $(this)
                              .removeClass("active")
                            ...
                            $(".active.bt-select-input")
                              .removeClass("active")
                              .find(".bt-down-icon")
                              .css("transform", "rotate(-45deg)");
                            $(
                              ".bt-select-list li:contains(" + _choose + ")"
                            ).addClass("active");
                      ...
                      $(".nginx-add-format").on(
                        ...
                        function (e) {
                          ...
                          $(".active.bt-select-input")
                            .find(".bt-select-val")
                            .attr("data-active", _value)
                            .text(_value);
                          $(
                            ".nginx-add-format .bt-select-list,.active.bt-select-input"
                          ).removeClass("active");
                        }
                      );
                      ...
                      $(".nginx-add-format").on(
                        ...
                        function (e) {
                          if ($(".del-format").length == 1) {
                            layer.msg("This is the last parameter.", {
                              icon: 2,
                            });
                            return false;
                          }
                    ...
                    yes: function (index, layero) {
                      if (
                        ...
                        layer.msg("The format name cannot be empty!", {
                          icon: 2,
                        });
                        return false;
                      }
                      var log_format = [];
                      $(".nginx-add-format .format-table .bt-select-val").each(
                        function () {
                      ...
                      var format_data = {
                        log_format_name: $(
                          ".nginx-add-format [name=log_format_name]"
                        ).val(),
                      ...
                      bt.send(
                        ...
                        "config/add_nginx_access_log_format",
                        format_data,
                        function (res) {
                          layer.close(index);
                      ...
                      loadP = layer.confirm(
                        ...
                        function () {
                          ...
                          bt.send(
                            ...
                            "config/del_nginx_access_log_format",
                            { log_format_name: log_format_name },
                            function (res) {
                              if (res.status)
                                $(
                                  '.bt-soft-menu p:contains("Logs format")'
                                ).click();
                              layer.msg(res.msg, { icon: res.status ? 1 : 2 });
                            }
                          );
                    ...
                    bt.send(
                      ...
                      function (res) {
                        ...
                        Object.keys(res.site_list).map(function (key) {
                          _site_ul +=
                            '<li style="padding: 10px"><div class="bt_checkbox_groups' +
                            (res.site_list[key] ? " active" : "") +
                            '" data-val="' +
                            key +
                            '"></div>' +
                            key +
                        ...
                        layer.open({
                          ...
                          yes: function (index, layero) {
                            var sites = [];
                            $(
                              ".nginx-add-site .format-site-list .bt_checkbox_groups.active"
                            ).each(function () {
                            ...
                            bt.send(
                              ...
                              "config/set_format_log_to_website",
                              format_data,
                              function (res) {
                                layer.close(index);
                                if (res.status)
      ...
      case "apache_format_log":
        ...
        bt.send(
          ...
          "config/get_httpd_access_log_format",
          {},
          function (rdata) {
            $(".soft-man-con").html(
              "<button class='btn btn-success btn-sm mb15 table-add-format'>Add format</button><div class='divtable' style='max-height: 570px;overflow: auto;'><table id='tab-nginx-logs-format' class='table table-hover'><thead><tr><th width='15%'>Name</th><th>Format</th><th width='120' style='text-align:right;'>Opt</th></tr></thead><tbody></tbody></table></div>"
            ...
            bt.send(
              ...
              "config/get_httpd_access_log_format_parameter",
              {},
              function (res) {
                loadT.close();
                var _format_ul = '<ul class="bt-select-list">';
                  ...
                  layer.open({
                    ...
                    yes: function (index, layero) {
                      ...
                      bt.send(
                        ...
                        "config/add_httpd_access_log_format",
                        format_data,
                        function (res) {
                          layer.close(index);
                          if (res.status)
                      ...
                      loadP = layer.confirm(
                        ...
                        function () {
                          ...
                          bt.send(
                            ...
                            "config/del_httpd_access_log_format",
                            { log_format_name: log_format_name },
                            function (res) {
                              if (res.status)
                                $(
                        ...
                        layer.open({
                          ...
                          yes: function (index, layero) {
                            ...
                            bt.send(
                              ...
                              "config/set_httpd_format_log_to_website",
                              format_data,
                              function (res) {
                                layer.close(index);
                                if (res.status)
      ...
      case "nginx_set":
        ...
        bt.send("GetNginxValue", "config/GetNginxValue", {}, function (rdata) {
          ...
          $(".soft-man-con")
            .empty()
            .append('<div class="set_nginx_config"></div>');
          for (var i = 0; i < form_datas.length; i++) {
            bt.render_form_line(
              form_datas[i],
              "",
      ...
      case "apache_set":
        ...
        bt.send("GetNginxValue", "config/GetApacheValue", {}, function (rdata) {
          ...
          $(".soft-man-con")
            .empty()
            .append('<div class="set_Apache_config"></div>');
          for (var i = 0; i < form_datas.length; i++) {
            bt.render_form_line(
              form_datas[i],
              "",
        ...
        bt.send(
          ...
          function (rdata) {
            ...
            if (key == "memcached_set") {
              ...
              arr["curr_connections"] = [
                rdata.curr_connections,
                lan.soft.curr_connect,
              ];
              ...
              arr["bytes_read"] = [
                ToSize(rdata.bytes_read),
                lan.soft.request_size_total,
              ];
              arr["bytes_written"] = [
                ToSize(rdata.bytes_written),
                lan.soft.send_size_total,
              ];
      ...
      case "phpmyadmin_php":
        bt.send("GetPHPVersion", "site/GetPHPVersion", {}, function (rdata) {
          ...
          for (var i = 0; i < rdata.length; i++) {
            optionSelect =
              rdata[i].version == sdata.ext.phpversion ? "selected" : "";
            body +=
              "<option value='" +
              rdata[i].version +
              "' " +
        ...
        bt.soft.php.get_config(version, function (rdata) {
            ...
            columns: [
              ...
              },
              {
                ...
                templet: function (item) {
                  return (
                    '<span class="ico-' +
                    (item.status ? "start" : "stop") +
                    " glyphicon glyphicon-" +
                    (item.status ? "ok" : "remove") +
                    '"></span>'
                  );
                },
              },
              {
                ...
                templet: function (item) {
                  ...
                  if (
                    item["task"] == "-1" &&
                    item.phpversions.indexOf(version) != -1
                  ) {
                    opt =
                      '<a style="color:green;" href="javascript:messagebox();">' +
                      lan.soft.the_install +
                      "</a>";
                  } else if (
                    item["task"] == "0" &&
                    item.phpversions.indexOf(version) != -1
                    ...
                    opt =
                      '<a style="color:#C0C0C0;" href="javascript:messagebox();">' +
                      lan.soft.sleep_install +
                      "</a>";
                  } else if (item.status) {
                    opt =
                      '<a style="color:red;" data-name="' +
                      item.name +
                      '" data-title="' +
                      item.title +
                      '" class="lib-uninstall" href="javascript:;">' +
                      lan.soft.uninstall +
                      "</a>";
                  }
                  return opt;
          ...
          $("a").click(function () {
            ...
            if (_obj.hasClass("lib-uninstall")) {
              bt.soft.php.un_install_php_lib(
                version,
                _obj.attr("data-name"),
                _obj.attr("data-title"),
                function (rdata) {
                  setTimeout(function () {
        ...
        $.post(
          ...
          function (php_info) {
            con +=
              ...
              "</button>";
            con +=
              '<div class="php_info_group"><p>' +
              lan.soft.php_base_info +
              " </p>";
            con +=
              '<table id="tab_php_status" class="table table-hover table-bordered" style="margin:0;padding:0">';
            con +=
              "<tr><td>" +
              lan.soft.version +
              "</td><td>" +
              php_info.phpinfo.php_version +
              "</td><td>" +
              lan.soft.install_path +
              "</td><td>" +
              php_info.phpinfo.php_path +
              "</td></tr>";
            con +=
              '<tr><td>php.ini</td><td colspan="3">' +
              php_info.phpinfo.php_ini +
              "</td></tr>";
            con +=
              "<tr><td>" +
              lan.soft.loaded +
              '</td><td colspan="3">' +
              php_info.phpinfo.modules +
              "</td></tr>";
            con += "</table></div>";
              ...
              bt.send(
                ...
                function (rdata) {
                  ...
                  bt.open({
                    ...
                    shadeClose: true,
                    content:
                      '<div style="white-space: pre-wrap;padding:0 10px;">' +
                      content +
                      "</div>",
                  });
                }
        ...
        bt.soft.php.get_php_config(version, function (rdata) {
          ...
          for (var i = 0; i < rdata.length; i++) {
            ...
            var ibody =
              '<input style="width: ' +
              w +
              'px;" class="bt-input-text mr5" name="' +
              rdata[i].name +
              '" value="' +
              rdata[i].value +
              '" type="text" >';
      ...
      case "set_upload_limit":
        bt.soft.php.get_limit_config(version, function (ret) {
          ...
          var tabCon = $(".soft-man-con")
            .empty()
            .append("<div class='set_upload_limit'></div>");
      ...
      case "set_timeout_limit":
        bt.soft.php.get_limit_config(version, function (ret) {
          ...
          var tabCon = $(".soft-man-con")
            .empty()
            .append("<div class='set_timeout_limit bt-form'></div>");
          for (var i = 0; i < datas.length; i++) {
            var _form_data = bt.render_form_line(datas[i]);
      ...
      case "set_dis_fun":
        bt.soft.php.get_config(version, function (rdata) {
          ...
          _bt_form.append(
            "<div class='divtable mtb15' style='height:500px;overflow:auto'><table id=\"blacktable\" class='table table-hover' width='100%' style='margin-bottom:0'></table><div>"
          );
          var _tab = bt.render({
            table: "#blacktable",
            data: list,
            columns: [
              ...
              },
              {
                ...
                templet: function (item) {
                  ...
                  new_disable_functions.splice(
                    $.inArray(item.name, new_disable_functions),
                    1
                  );
                  return (
              ...
              callback: function (ldata) {
                bt.pub.get_menm(function (memInfo) {
                  ...
                  if (limit_children < parseInt(ldata.max_children)) {
                    layer.msg(
                      lan.soft.php_child_process.replace("{1}", limit_children),
                      {
                        icon: 2,
                      }
                    );
                  ...
                  if (
                    parseInt(ldata.max_children) <
                    parseInt(ldata.max_spare_servers)
                  ) {
                  ...
                  if (
                    parseInt(ldata.min_spare_servers) >
                    parseInt(ldata.start_servers)
                  ) {
                  ...
                  if (
                    parseInt(ldata.max_children) < parseInt(ldata.start_servers)
                  ) {
                  ...
                  if (
                    parseInt(ldata.max_children) < 1 ||
                    parseInt(ldata.start_servers) < 1 ||
                    parseInt(ldata.min_spare_servers) < 1 ||
                    parseInt(ldata.max_spare_servers) < 1
                  ) {
      ...
      case "get_php_status":
        bt.soft.php.get_php_status(version, function (rdata) {
          ...
          arr[lan.bt.php_manager] =
            rdata["process manager"] == "dynamic"
              ? lan.bt.dynamic
              : lan.bt.static;
          ...
          var con =
            "<div style='overflow:auto;'><table id='tab_php_status' class='table table-hover table-bordered' style='margin:0;padding:0'></table></div>";
          $(".soft-man-con").html(con);
        ...
        bt.soft.php.get_php_session(version, function (res) {
          $(".soft-man-con").html(
            '<div class="conf_p">' +
              ...
              (version != "52"
                ? '<option value="redis" ' +
                  (res.save_handler == "redis" ? "selected" : "") +
                  ">redis</option>"
                : "") +
              (version != "73"
                ? '<option value="memcache" ' +
                  (res.save_handler == "memcache" ? "selected" : "") +
                  ">memcache</option>"
                : "") +
          ...
          if (res.save_handler == "files") {
            ...
            $(".clear_conter")
              .html(lan.soft.only_files_storage_mode_can_clear)
      ...
      case "get_fpm_logs":
        bt.soft.php.get_fpm_logs(version, function (logs) {
          var phpCon =
            '<textarea readonly="" style="margin: 0px;width: 600px;height: 620px;background-color: #333;color:#fff; padding:0 5px" id="error_log">' +
            logs.msg +
            "</textarea>";
          $(".soft-man-con").html(phpCon);
          var ob = document.getElementById("error_log");
      ...
      case "get_redis_status":
        bt.soft.redis.get_redis_status(function (rdata) {
          var hit = (
            (parseInt(rdata.keyspace_hits) /
              (parseInt(rdata.keyspace_hits) +
                parseInt(rdata.keyspace_misses))) *
            100
          ).toFixed(2);
          var arrs = [];
          arrs["uptime_in_days"] = [rdata.uptime_in_days, lan.soft.run_days];
          arrs["tcp_port"] = [rdata.tcp_port, lan.soft.curr_listen_port];
          arrs["connected_clients"] = [
            rdata.connected_clients,
            lan.soft.connected_clients,
          ];
          arrs["used_memory_rss"] = [
            bt.format_size(rdata.used_memory_rss),
            lan.soft.used_memory_rss,
          ];
          arrs["used_memory"] = [
            bt.format_size(rdata.used_memory),
            lan.soft.used_memory,
          ];
          arrs["mem_fragmentation_ratio"] = [
            rdata.mem_fragmentation_ratio,
            lan.soft.mem_fragmentation_ratio,
          ];
          arrs["total_connections_received"] = [
            rdata.total_connections_received,
            lan.soft.total_connections_received,
          ];
          arrs["total_commands_processed"] = [
            rdata.total_commands_processed,
            lan.soft.total_commands_processed,
          ];
          arrs["instantaneous_ops_per_sec"] = [
            rdata.instantaneous_ops_per_sec,
            lan.soft.instantaneous_ops_per_sec,
          ];
          arrs["keyspace_hits"] = [rdata.keyspace_hits, lan.soft.keyspace_hits];
          arrs["keyspace_misses"] = [
            rdata.keyspace_misses,
            lan.soft.keyspace_misses,
          ];
          arrs["hit"] = [hit, lan.soft.db_his];
          arrs["latest_fork_usec"] = [
            rdata.latest_fork_usec,
            lan.soft.latest_fork_usec,
          ];
      ...
      case "openliMa_set":
        ...
        $.post("config?action=get_ols_value", function (rdata) {
          ...
          var tabCon = $(".soft-man-con").empty();
          tabCon.append(
            '<div class="openlite_set">' + _mlist_data + "</div></div>"
          );
          var datas = {
            title: " ",
            ...
            items: [
              ...
              },
              {
                ...
                callback: function (ldata) {
                  ...
                  ldata["enableGzipCompress"] = $("#enableGzipCompress").prop(
                    "checked"
                  )
                    ? 1
                    : 0;
  ...
  input_zip: function (plugin_name, tmp_path, data) {
    bt.soft.show_speed_window(
      { title: "Installing, this may take a few minutes...", status: true },
      function () {
        $.post(
          "/plugin?action=input_zip",
          { plugin_name: plugin_name, tmp_path: tmp_path },
          function (rdata) {
            layer.closeAll();
            if (rdata.status) {
              soft.get_list();
...
function AddSite(codename, title) {
  ...
  if (checkDomain.length < 1) {
    layer.msg(
      "The domain name is not in the correct format. Please re-enter!",
      {
        icon: 2,
      }
    );
  ...
  var data =
    ...
    "&webname=" +
    domain +
    "&ftp=false&sql=true&address=localhost&codeing=utf8&version=" +
    php_version;
  $.post("/site?action=AddSite", data, function (ret) {
    ...
    var pdata =
      ...
      php_version;
    var loadT = layer.msg(
      '<div class="depSpeed">Submitting <img src="/static/img/ing.gif"></div>',
      {
        icon: 16,
        time: 0,
        shade: [0.3, "#000"],
    ...
    $.post("/deployment?action=SetupPackage", pdata, function (rdata) {
      ...
      sqlData +=
        "<p><span>Visit site：</span><a class='btlink' href='http://" +
        mainDomain +
        rdata.msg.success_url +
        "' target='_blank'>http://" +
        mainDomain +
        rdata.msg.success_url +
        "</a></p>";
...
function onekeyCodeSite(codename, versions, title, enable_functions) {
  $.post("/site?action=GetPHPVersion", function (rdata) {
    ...
    if (enable_functions.length > 2) {
      layer.msg(
        "<span style='color:red'>Note: The following functions will be released when deploying this project.:<br> " +
          enable_functions +
          "</span>",
        {
          icon: 7,
    ...
    $("#mainDomain").on("input", function () {
      ...
      if ($("#inputPath").val().substr(0, defaultPath.length) == defaultPath)
        $("#inputPath").val(defaultPath + "/" + ress);
...
var score = {
  ...
  get_score_info: function (obj, callback) {
    var loadT = layer.msg(
      '<div class="depSpeed">Getting comment information <img src="/static/img/ing.gif"></div>',
      {
        icon: 16,
        time: 0,
        shade: [0.3, "#000"],
  ...
  render_score_info: function (obj, callback) {
    ...
    obj.limit_num == undefined
      ? (config.limit_num = "")
      : (config.limit_num = obj.limit_num);
    score.get_score_info(config, function (res) {
      ...
      $(".comment_rate").text(
        res.total !== 0
          ? ((_split_score[0] + _split_score[1]) / res.total).toFixed(2) * 100 +
              "%"
          : "0%"
      );
      for (var i = 0; i < 5; i++) {
      ...
      for (var j = 0; j < _data.length; j++) {
        _html +=
          ...
          (getLength(_data[j].ps) > 65
            ? reBytesStr(_data[j].ps, 65) +
              '...&nbsp;<a href="javascript:;" class="btlink">Details</a>'
            : _data[j].ps) +
          "</div>\
                </div>";
      ...
      if (res.total > 10 && _data.length === 10) {
        _html +=
          '<div class="comment_box get_next_page"><span class="glyphicon glyphicon-chevron-down" aria-hidden="true"></span>Click for more comments</div>';
      }
  ...
  set_score_info: function (obj, callback) {
    var loadT = layer.msg(
      '<div class="depSpeed">Submitting comment <img src="/static/img/ing.gif"></div>',
      {
        icon: 16,
        time: 0,
        shade: [0.3, "#000"],
      ...
      success: function (index, layero) {
        ...
        $(".score_icon_group span").click(function () {
          ...
          if (
            ...
            $(this).nextAll(".active").length == 0 &&
            _active
          ) {
            $(".edit_view").addClass("active");
            $(this).removeClass("active").nextAll().removeClass("active");
            $(".score_icon_group_tips").html(
              "Click on the selection icon to rate 1-5 stars"
            );
            $(".score_icon_group").attr("data-icon", 0);
          } else {
        ...
        $(".comment_content").on("click", ".comment_box", function () {
          if (!$(this).hasClass("get_next_page")) {
            ...
            layer.open({
              ...
              shadeClose: false,
              content:
                '<div class="score_details" >' + $(this).html() + "</div>",
              success: function (index, layers) {
                $(".score_details .comment_box_content").html(
                  score.data[_index]["ps"]
                );
              },
            });
          }
