...
var index = {
    ...
    set_status: function (obj, status, val) {
      ...
      if (status == 2) {
        ...
        for (var i = 0; i < btlen; i++) {
          setTimeout(
            index.set_val(obj.parents("li"), { usage: btlen - i }),
            i * 30
          );
        }
        ...
        obj
          ...
          .html(
            _div
              .append(
                lan.index.memre_ok_0 + ' <img src="/static/img/ings.gif">'
              )
              .html()
          );
      } else if (status == 1) {
    ...
    _this.reander_system_info(function (rdata) {
      ...
      $("#memChart")
        .hover(
          function () {
            $(this).append(
              '<div class="mem_mask shine_green" title="Click to free RAM"><div class="men_inside_mask"></div><div class="mem-re-con" style="display:block"></div></div>'
            );
        ...
        .click(function () {
          ...
          bt.show_confirm(
            lan.index.mem_release_sure,
            '<font style="color:red;">' +
              lan.index.mem_release_warn +
              "</font>",
            function () {
              _this.release = true;
      ...
      for (var i = 0; i < rdata.disk.length; i++) {
        var disk = rdata.disk[i],
          texts = "<strong>" + lan.index.base_info + "</strong></br>";
    ...
    setTimeout(function () {
      bt.system.check_update(function (rdata) {
        ...
        if (rdata.status !== false) {
          $("#toUpdate a").html(
            lan.index.update +
              '<i style="display: inline-block; color: red; font-size: 40px;position: absolute;top: -35px; font-style: normal; right: -8px;">.</i>'
          );
  ...
  get_server_info: function (info) {
    ...
    if (memFree < 64) {
      $("#messageError").show();
      $("#messageError").append(
        '<p><span class="glyphicon glyphicon-alert" style="color: #ff4040; margin-right: 10px;">' +
  ...
  render_disk: function (data) {
    if (data.items.length > 0) {
      ...
      for (var i = 0; i < data.items.length; i++) {
        ...
        var html = "";
        html +=
          '<li class="col-xs-6 col-sm-3 col-md-3 col-lg-2 mtb20 circle-box text-center diskbox">';
        html += '<h3 class="c9 f15">' + item.name + "</h3>";
        html += '<div class="cicle">';
        html +=
          '<div class="bar bar-left"><div class="bar-left-an bar-an"></div></div>';
        html +=
          '<div class="bar bar-right"><div  class="bar-right-an bar-an"></div></div>';
        ...
        if (item.masks) {
          var mask = "";
          for (var j = 0; j < item.masks.length; j++)
            mask += item.masks[j].title + ": " + item.masks[j].value + "<br>";
          _li.data("mask", mask);
          _li.find(".cicle").hover(
            function () {
              var _this = $(this);
              layer.tips(_this.parent().data("mask"), _this, {
                time: 0,
                tips: [1, "#999"],
              });
            },
  ...
  set_val: function (_li, obj) {
    ...
    if (obj.usage > 50) {
      ...
      setTimeout(function () {
        _li
          ...
          .css({
            transform:
              "rotate(" + (((obj.usage - 50) / 100) * 360 - 135) + "deg)",
            transition: "transform 750ms linear",
          });
      }, 760);
    } else {
      if (parseInt(_li.find(".occupy span").html()) > 50) {
        setTimeout(function () {
          _li
            .find(".bar-right-an")
            .css({
              transform: "rotate(" + ((obj.usage / 100) * 360 - 135) + "deg)",
              transition: "transform 750ms linear",
            });
        }, 760);
        setTimeout(function () {
    ...
    if (obj.color) {
      _li
        .find(".cicle .bar-left-an")
        .css(
          "border-color",
          "transparent transparent " + obj.color + " " + obj.color
        );
      _li
        .find(".cicle .bar-right-an")
        .css(
          "border-color",
          obj.color + " " + obj.color + " transparent transparent"
        );
  ...
  init_chart_view: function () {
    // 所有图表对象装进chart_view
    this.chart_view["load"] = echarts.init(
  ...
  set_chart_data: function () {
    ...
    for (var i = 0; i < this.chart_view.disk.length; i++) {
      ...
      this.series_option.series[0].data[0].itemStyle.color =
        this.chart_json.disk[i].color;
      this.series_option.series[0].data[0].itemStyle.borderColor =
        this.chart_json.disk[i].color;
      this.series_option.series[0].progress.itemStyle.borderColor =
        this.chart_json.disk[i].color;
      ...
      $("#disk" + i).text(
        this.chart_result.disk[i].size[1] +
          " / " +
          this.chart_result.disk[i].size[0]
      );
  ...
  chart_active: function (name) {
    ...
    switch (name) {
      ...
      case "mem":
        val =
          this.chart_result.mem.memRealUsed +
          " / " +
          this.chart_result.mem.memTotal +
          "(MB)";
        break;
    }
  ...
  get_index_list: function () {
    bt.soft.get_index_list(function (rdata) {
      ...
      for (var i = 0; i < rlen; i++) {
        if (rdata[i].setup) {
          ...
          if (rdata[i].admin) {
            ...
            clickName =
              "onclick=\"soft.set_soft_config('" + rdata[i].name + "')\"";
          }
      ...
      $("#indexsoft").dragsort({
        ...
        dragEnd: saveOrder,
        placeHolderTemplate:
          "<div class='col-sm-3 col-md-3 col-lg-3 dashed-border'></div>",
      });
    ...
    bt.system.check_update(function (rdata) {
        ...
        var result = rdata,
          ...
          loading = bt.open({
            ...
            title:
              "[Linux" +
              (rdata.msg.is_beta == 1
                ? lan.index.test_version
                : lan.index.final_version) +
              "]-" +
            ...
            content:
              '<div class="setchmod bt-form">\
                                <div class="update_title"><i class="layui-layer-ico layui-layer-ico1"></i><span>' +
              ...
              lan.index.bt_linux +
              (rdata.msg.is_beta == 1
                ? lan.index.test_version + " " + rdata.msg.beta.version
                : lan.index.final_version + " " + rdata.msg.version) +
              "</a>&nbsp;&nbsp;" +
              lan.index.release_time +
              (rdata.msg.is_beta == 1
                ? rdata.msg.beta.uptime
                : rdata.msg.uptime) +
              '</div>\
              ...
              (result.msg.is_beta != 1
                ? result.msg.beta.version
                : result.msg.version) +
              "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
              lan.index.update_time +
              "&nbsp;&nbsp;" +
              (is_beta != 1 ? result.msg.beta.uptime : result.msg.uptime) +
              "</div>\
                                        " +
              (is_beta !== 1
                ? "<span>" +
                  ...
                  "</a></span>"
                : "<span>" +
                  lan.index.change_final_click +
                  '<a href="javascript:;" onclick="index.to_not_beta()" class="btlink btn_update_testPanel">&nbsp;&nbsp;' +
              ...
              lan.public.cancel +
              '</button>\
                                    <button type="button" class="btn btn-success btn-sm btn-title btn_update_panel" onclick="layer.closeAll()">' +
              lan.public.know +
              "</button>\
                                </div>\
                            </div>\
      ...
      if (rdata.status === true) {
        ...
        var loading = bt.open({
          ...
          closeBtn: 2,
          content:
            '<div class="setchmod bt-form" style="padding-bottom:50px;">\
                                    <div class="update_title"><i class="layui-layer-ico layui-layer-ico0"></i><span>' +
            ...
            rdata.version +
            "</a></br>" +
            lan.index.update_date +
            (result.msg.is_beta == 1
              ? result.msg.beta.uptime
              : result.msg.uptime) +
            '</div>\
            ...
            lan.public.cancel +
            '</button>\
                                        <button type="button" class="btn btn-success btn-sm btn-title btn_update_panel" onclick="index.to_update()" >' +
            lan.index.update_go +
            "</button>\
                                    </div>\
                                </div>\
  ...
  to_not_beta: function () {
    bt.show_confirm(
      lan.index.change_final,
      lan.index.change_test_to_final,
      function () {
    ...
    bt.send("get_beta_logs", "ajax/get_beta_logs", {}, function (data) {
      ...
      function settime(val) {
        if (countdown == 0) {
          ...
          $("#update_time").text(
            lan.index.second_left_of_click.replace("{1}", countdown)
          );
          countdown--;
          setTimeout(function () {
            settime(val);
      ...
      $(".btn_update_panel_beta").click(function () {
        bt.show_confirm(
          lan.index.update_test_version,
          lan.index.check_test_version_detail,
          function () {
            var loading = bt.load();
    ...
    setTimeout(function () {
      $(".rebt-con a").click(function () {
        ...
        switch (type) {
          case "panel":
            layer.confirm(
              lan.index.panel_reboot_msg,
              { title: lan.index.panel_reboot_title, closeBtn: 2, icon: 3 },
              function () {
                var loading = bt.load();
                interval_stop = true;
          ...
          case "server":
            ...
            setTimeout(function () {
              ...
              $(".WSafeRestart").click(function () {
                var body =
                  '<div class="SafeRestartCode pd15" style="line-height:26px"></div>';
                $(".bt-window-restart").html(body);
                $(".SafeRestartCode").append(
                  "<p>" + lan.index.reboot_msg_1 + "</p>"
                );
                bt.pub.set_server_status_by(
                  "name={{session['webserver']}}&type=stop",
                  function (r1) {
                    $(".SafeRestartCode p").addClass("c9");
                    $(".SafeRestartCode").append(
                      "<p>" + lan.index.reboot_msg_2 + "...</p>"
                    );
                    bt.pub.set_server_status_by(
                      ...
                      function (r2) {
                        $(".SafeRestartCode p").addClass("c9");
                        $(".SafeRestartCode").append(
                          "<p>" + lan.index.reboot_msg_3 + "...</p>"
                        );
                        bt.system.root_reload(function (rdata) {
                          $(".SafeRestartCode p").addClass("c9");
                          $(".SafeRestartCode").append(
                            "<p>" + lan.index.reboot_msg_4 + "...</p>"
                          );
  ...
  open_log: function () {
    ...
    $.get(
      "https://www.bt.cn/Api/getUpdateLogs?type=" + bt.os,
      function (rdata) {
        var body = "";
  ...
  get_warning_list: function (active, callback) {
    ...
    bt.send("get_list", "warning/get_list", obj, function (res) {
      if (res.status !== false) {
        ...
        $(".warning_scan_ps").html(
          that.warning_num > 0
            ? "This scan check <i>" +
                that.warning_num +
                "</i> risks, please repair them in time!"
            : "There is no risk item detected in this scan, please keep it!"
        );
        if (callback) callback(res);
      }
    });
    ...
    function reader_warning_list(data) {
      ...
      bt.each(arry, function (index, item) {
        ...
        html +=
          ...
          (function (index, item) {
            var htmls =
              '<ul class="module_details_list ' +
              (item[0] == "risk" && that.warning_num > 0 ? "active" : "") +
              '">';
            bt.each(data_item, function (indexs, items) {
              ...
              htmls +=
                ...
                (items.taking > 1
                  ? items.taking + "Sec"
                  : (items.taking * 1000).toFixed(2) + "ms") +
                "）</i></span>" +
      ...
      $(".warning_scan_time").html(
        "Checked: &nbsp;" + bt.format_data(scan_time)
      );
    }
    bt.open({
      type: "1",
      ...
      content:
        ...
        (that.warning_num > 0
          ? "This scan check <i>" +
            that.warning_num +
            "</i> risks, please repair them in time!"
          : "This scan check no risks, please keep it!") +
        "</span>" +
      ...
      success: function () {
        $(".warning_again_scan").click(function () {
          var loadT = layer.msg("Re detecting security risks, please wait...", {
            icon: 16,
          });
          ...
          function () {
            ...
            if (parseInt($(this).find(".module_num").text()) > 0) {
              if (_list.hasClass("active")) {
                ...
                var details_list = _list
                  .parent()
                  .siblings()
                  .find(".module_details_list");
            ...
            case 1:
              if (data.type != "ignore") {
                bt.confirm(
                  {
                    title: "Ignore risk",
                    msg: "Confirm to ignore【" + data.title + "】risk?",
                  },
                  function () {
  ...
  waring_check_find: function (model_name, callback) {
    var loadT = layer.msg("Detecting the specified module, please wait...", {
      icon: 16,
      time: 0,
    });
    bt.send(
  ...
  warning_set_ignore: function (model_name, callback) {
    var loadT = layer.msg("Setting the specified module, please wait...", {
      icon: 16,
      time: 0,
    });
    bt.send(
