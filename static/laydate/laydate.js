!(function () {
    ...
    ready = {
      ...
      link: function (href, fn, cssname) {
        ...
        (function poll() {
          ...
          parseInt(ready.getStyle(document.getElementById(id), "width")) ===
          1989
            ? fn()
            : setTimeout(poll, 100);
        })();
    ...
    lay.each(["min", "max"], function (i, item) {
      ...
      if (typeof options[item] === "number") {
        var day = options[item],
          ...
          STAMP = 86400000,
          thisDate = new Date(
            day ? (day < STAMP ? time + day * STAMP : day) : time
          );
        ymd = [
  ...
  Class.prototype.position = function () {
    ...
    if (top + elemHeight + margin > winArea()) {
      top =
        rect.top > elemHeight ? rect.top - elemHeight : winArea() - elemHeight;
      top = top - margin * 2;
    }
    if (options.position) {
      ...
      if (type === "month") {
        ...
        if (type === "time") {
          var setTimeStatus = function () {
            ...
            if (!options.range) {
              that.limit(
                lay(that.footer).find(ELEM_CONFIRM),
                that[startEnd],
                0,
                ["hours", "minutes", "seconds"]
              );
